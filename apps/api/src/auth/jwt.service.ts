...
export class JwtService {
  ...
  verifyJwt(token: string): AuthUser {
    ...
    const payload = JSON.parse(Buffer.from(encodedPayload, 'base64url').toString('utf8')) as AuthUser & { exp: number };

    if (!payload.id || !payload.username || payload.exp < Math.floor(Date.now() / 1000)) {
      throw new UnauthorizedException('Invalid token');
    }
